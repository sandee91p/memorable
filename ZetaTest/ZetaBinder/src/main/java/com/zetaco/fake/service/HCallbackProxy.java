...
import org.android.app.servertransaction.ClientTransaction;
import org.android.app.servertransaction.LaunchActivityItem;
...
public class HCallbackProxy implements IInjectHook, Handler.Callback {
    ...
    private Handler.Callback getHCallback() {
        return org.android.os.Handler.mCallback.get(getH());
    }
    ...
    public void injectHook() {
        ...
        org.android.os.Handler.mCallback.set(getH(), this);
    }
    ...
    private boolean handleLaunchActivity(Object client) {
        ...
        if (activityInfo != null) {
            if (BActivityThread.getAppConfig() == null) {
                ZetaBCore.getBActivityManager().restartProcess(activityInfo.packageName, activityInfo.processName, stubRecord.mUserId);

                Intent launchIntentForPackage = ZetaBCore.getBPackageManager().getLaunchIntentForPackage(activityInfo.packageName, stubRecord.mUserId);
            ...
            ZetaBCore.getBActivityManager().onActivityCreated(taskId, token, stubRecord.mActivityToken);
            ...
            if (Build.VERSION.SDK_INT == Build.VERSION_CODES.S || (Build.VERSION.SDK_INT == Build.VERSION_CODES.R && Build.VERSION.PREVIEW_SDK_INT == 1)) {
                Object record = ActivityThread.getLaunchingActivity.call(ZetaBCore.mainThread(), token);
    ...
    private boolean handleCreateService(Object data) {
        if (BActivityThread.getAppConfig() != null) {
            ...
            if (!serviceInfo.name.equals(ProxyManifest.getProxyService(BActivityThread.getAppPid()))
                ...
                ZetaBCore.getBActivityManager().startService(intent, null, false, BActivityThread.getUserId());
                return true;
            }
        }
        return false;

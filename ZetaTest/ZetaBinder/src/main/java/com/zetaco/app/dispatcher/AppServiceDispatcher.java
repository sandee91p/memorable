package com.zetaco.app.dispatcher;
...
import com.zetaco.entity.ServiceRecord;
import com.zetaco.entity.UnbindRecord;
import com.zetaco.proxy.record.ProxyServiceRecord;
...
public class AppServiceDispatcher {
    ...
    private final Handler mHandler = ZetaBCore.get().getHandler();
    ...
    public void onUnbind(Intent proxyIntent) {
        ...
        try {
            UnbindRecord unbindRecord = ZetaBCore.getBActivityManager().onServiceUnbind(proxyIntent, BActivityThread.getUserId());
            ...
            if (destroy || record.decreaseConnectionCount(intent)) {
                if (destroy) {
                    ...
                    ZetaBCore.getBActivityManager().onServiceDestroy(proxyIntent, BActivityThread.getUserId());
        ...
        if (record.getService() != null) {
            ...
            try {
                if (destroy) {
                    ...
                    ZetaBCore.getBActivityManager().onServiceDestroy(intent, BActivityThread.getUserId());
                    mService.remove(new Intent.FilterComparison(intent));
                }
            } catch (Throwable e) {
                e.printStackTrace();

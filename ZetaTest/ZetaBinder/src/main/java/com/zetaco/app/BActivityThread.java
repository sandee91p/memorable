...
import org.android.app.ActivityThreadNMR1;
import org.android.app.ActivityThreadQ;
import org.android.app.ContextImpl;
import org.android.app.LoadedApk;
import org.android.graphics.Compatibility;
import org.android.security.net.config.NetworkSecurityConfigProvider;
import org.com.android.internal.content.ReferrerIntent;
import org.dalvik.system.VMRuntime;

import com.zetaco.ZetaBCore;
import com.zetaco.app.configuration.AppLifecycleCallback;
import com.zetaco.app.dispatcher.AppServiceDispatcher;
import com.zetaco.core.CrashHandler;
import com.zetaco.core.IBActivityThread;
import com.zetaco.core.IOCore;
import com.zetaco.core.NativeCore;
import com.zetaco.core.env.VirtualRuntime;
import com.zetaco.core.system.user.BUserHandle;
import com.zetaco.entity.AppConfig;
import com.zetaco.entity.am.ReceiverData;
...
import com.zetaco.utils.compat.StrictModeCompat;
...
public class BActivityThread extends IBActivityThread.Stub {
    ...
    private final Handler mH = ZetaBCore.get().getHandler();
    ...
    public static int getCallingBUid() {
        return getAppConfig() == null ? ZetaBCore.getHostUid() : getAppConfig().callingBUid;
    }
    ...
    public Service createService(ServiceInfo serviceInfo, IBinder token) {
        ...
        try {
            Context context = ZetaBCore.getContext().createPackageContext(serviceInfo.packageName,
                    Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);

            ContextImpl.setOuterContext.call(context, service);
            org.android.app.Service.attach.call(service, context, ZetaBCore.mainThread(), serviceInfo.name, token, mInitialApplication,
    ...
    public JobService createJobService(ServiceInfo serviceInfo) {
        ...
        try {
            Context context = ZetaBCore.getContext().createPackageContext(serviceInfo.packageName, Context.CONTEXT_INCLUDE_CODE |
                    Context.CONTEXT_IGNORE_SECURITY);

            ContextImpl.setOuterContext.call(context, service);
            org.android.app.Service.attach.call(service, context, ZetaBCore.mainThread(), serviceInfo.name,
    ...
    public synchronized void handleBindApplication(String packageName, String processName) {
        ...
        PackageInfo packageInfo = ZetaBCore.getBPackageManager().getPackageInfo(packageName, PackageManager.GET_PROVIDERS, BActivityThread.getUserId());
        ...
        Object boundApplication = ActivityThread.mBoundApplication.get(ZetaBCore.mainThread());
        ...
        try {
            ...
            ActivityThread.mInitialApplication.set(ZetaBCore.mainThread(), mInitialApplication);
            ContextCompat.fix((Context) ActivityThread.getSystemContext.call(ZetaBCore.mainThread()));
    ...
    public static Context createPackageContext(ApplicationInfo info) {
        try {
            return ZetaBCore.getContext().createPackageContext(info.packageName, Context.CONTEXT_INCLUDE_CODE | Context.CONTEXT_IGNORE_SECURITY);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
            ...
            for (ProviderInfo providerInfo : provider) {
                try {
                    if (processName.equals(providerInfo.processName) || providerInfo.processName.equals(context.getPackageName()) || providerInfo.multiprocess) {
                        installProvider(ZetaBCore.mainThread(), context, providerInfo, null);
...
*/
    ...
    public IBinder getActivityThread() {
        return ActivityThread.getApplicationThread.call(ZetaBCore.mainThread());
    }
    ...
    public IBinder acquireContentProviderClient(ProviderInfo providerInfo) {
        ...
        for (String auth : split) {
            ContentProviderClient contentProviderClient = ZetaBCore.getContext().getContentResolver().acquireContentProviderClient(auth);
            IInterface iInterface = org.android.content.ContentProviderClient.mContentProvider.get(contentProviderClient);
    ...
    public void finishActivity(final IBinder token) {
        mH.post(() -> {
            Map<IBinder, Object> activities = ActivityThread.mActivities.get(ZetaBCore.mainThread());
            ...
            int resultCode = org.android.app.Activity.mResultCode.get(activity);
            Intent resultData = org.android.app.Activity.mResultData.get(activity);
            ActivityManagerCompat.finishActivity(token, resultCode, resultData);
            org.android.app.Activity.mFinished.set(activity, true);
        });
    }
    ...
    public void handleNewIntent(final IBinder token, final Intent intent) {
        mH.post(() -> {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP_MR1) {
                newIntent = ReferrerIntent._new.newInstance(ZetaBCore.getHostPkg());
            } else {
                newIntent = intent;
            }

            Object mainThread = ZetaBCore.mainThread();
    ...
    public void scheduleReceiver(ReceiverData data) {
        ...
        mH.post(() -> {
            ...
            try {
                ...
                org.android.content.BroadcastReceiver.setPendingResult.call(mReceiver, pendingResult);
                mReceiver.onReceive(baseContext, intent);

                BroadcastReceiver.PendingResult finish = org.android.content.BroadcastReceiver.getPendingResult.call(mReceiver);
                if (finish != null) {
                    finish.finish();
                }
                ZetaBCore.getBActivityManager().finishBroadcast(data.data);
    ...
    public static Activity getActivityByToken(IBinder token) {
        Map<IBinder, Object> iBinderObjectMap = ActivityThread.mActivities.get(ZetaBCore.mainThread());
    ...
    private void onBeforeCreateApplication(String packageName, String processName, Context context) {
        for (AppLifecycleCallback appLifecycleCallback : ZetaBCore.get().getAppLifecycleCallbacks()) {

...
import com.zetaco.entity.pm.InstalledPackage;
import com.zetaco.utils.AbiUtils;
...
public class BPackageManagerService extends IBPackageManagerService.Stub implements ISystemService {
    ...
    public BPackageManagerService() {
        ...
        ZetaBCore.getContext().registerReceiver(mPackageChangedHandler, filter, Context.RECEIVER_NOT_EXPORTED);
    }
    ...
    public ApplicationInfo getApplicationInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, ZetaBCore.getHostPkg())) {
            try {
                return ZetaBCore.getPackageManager().getApplicationInfo(packageName, flags);
    ...
    public List<ResolveInfo> queryIntentServices(Intent intent, int flags, int userId) {
        final String resolvedType = intent.resolveTypeIfNeeded(ZetaBCore.getContext().getContentResolver());
    ...
    public PackageInfo getPackageInfo(String packageName, int flags, int userId) {
        ...
        if (Objects.equals(packageName, ZetaBCore.getHostPkg())) {
            try {
                return ZetaBCore.getPackageManager().getPackageInfo(packageName, flags);
            } catch (PackageManager.NameNotFoundException e) {
                e.printStackTrace();
            }
            return null;
    ...
    private InstallResult installPackageAsUserLocked(String file, InstallOption option, int userId) {
        ...
        try {
            ...
            if (option.isFlag(InstallOption.FLAG_URI_FILE)) {
                ...
                InputStream inputStream = ZetaBCore.getContext().getContentResolver().openInputStream(Uri.parse(file));
            ...
            PackageInfo packageArchiveInfo = ZetaBCore.getPackageManager().getPackageArchiveInfo(apkFile.getAbsolutePath(), 0);
            ...
            if (!support) {
                String msg = packageArchiveInfo.applicationInfo.loadLabel(ZetaBCore.getPackageManager()) + "[" + packageArchiveInfo.packageName + "]";
                return result.installError(packageArchiveInfo.packageName,
                        msg + "\n" + (ZetaBCore.is64Bit() ? "The box does not support 32-bit Application" : "The box does not support 64-bit Application"));
            }

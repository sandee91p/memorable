...
public class AppSystemEnv {
    ...
    static {
        ...
        // sSystemPackages.add(ZetaBCore.getHostPkg());
    ...
    public static boolean isBlackPackage(String packageName) {
        if (ZetaBCore.get().isHideRoot() && sSuPackages.contains(packageName)) {
            return true;
        }
        return ZetaBCore.get().isHideXposed() && sXposedPackages.contains(packageName);
    }

...
public class GmsCore {
    ...
    private static InstallResult installPackages(Set<String> list, int userId) {
        ZetaBCore zetaBoxCore = ZetaBCore.get();
        for (String packageName : list) {
            if (zetaBoxCore.isInstalled(packageName, userId)) {
                continue;
            }

            try {
                ZetaBCore.getContext().getPackageManager().getApplicationInfo(packageName, 0);
            } catch (PackageManager.NameNotFoundException ignored) {
                continue;
            }

            InstallResult installResult = zetaBoxCore.installPackageAsUser(packageName, userId);
    ...
    private static void uninstallPackages(Set<String> list, int userId) {
        ZetaBCore zetaBoxCore = ZetaBCore.get();
        for (String packageName : list) {
            zetaBoxCore.uninstallPackageAsUser(packageName, userId);
        }
    }
    ...
    public static boolean isSupportGms() {
        try {
            ZetaBCore.getPackageManager().getPackageInfo(GMS_PKG, 0);
            return true;
        } catch (PackageManager.NameNotFoundException ignored) { }
        return false;
    }
    ...
    public static boolean isInstalledGoogleService(int userId) {
        return ZetaBCore.get().isInstalled(GMS_PKG, userId);
    }
}
...
import org.android.content.pm.ApplicationInfoL;
import org.android.content.pm.ApplicationInfoN;
import org.android.content.pm.SigningInfo;
...
public class PackageManagerCompat {
    ...
    public static PackageInfo generatePackageInfo(BPackage p, int flags, long firstInstallTime, long lastUpdateTime, BPackageUserState state, int userId) {
        ...
        try {
            base = ZetaBCore.getPackageManager().getPackageInfo(p.packageName, flags);
        ...
        if (BuildCompat.isPie()) {
            if ((flags & PackageManager.GET_SIGNING_CERTIFICATES) != 0) {
                if (base == null) {
                    org.android.content.pm.PackageParser.SigningDetails.signatures.set(null, p.mSigningDetails.signatures);
    ...
    public static ApplicationInfo generateApplicationInfo(BPackage p, int flags, BPackageUserState state, int userId) {
        ...
        try {
            baseApplication = ZetaBCore.getPackageManager().getApplicationInfo(ZetaBCore.getHostPkg(), flags);
        } catch (Exception e) {
            return null;
        }
        ...
        if (p.applicationInfo == null) {
            p.applicationInfo = ZetaBCore.getPackageManager().getPackageArchiveInfo(sourceDir, 0).applicationInfo;
        }
    ...
    public static Resources getResources(Context context, ApplicationInfo appInfo) {
        ...
        if (ps != null) {
            AssetManager assets = org.android.content.res.AssetManager._new.newInstance();
            org.android.content.res.AssetManager.addAssetPath.call(assets, ps.pkg.baseCodePath);
